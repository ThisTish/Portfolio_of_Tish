		...
		<>
			<main className={`bg-primary theme-${theme.class}`}>
				...
				<div className="flex text-highlight text-sm md:text-base py-3 lg:text-2xl absolute -top-1 right-0 md:top-0">
					...
					<button aria-label="Scroll to top" onClick={scrollToTop} className='text-secondary text-3xl md:text-4xl lg:text-5xl rounded-full bg-highlight bg-opacity-55 opacity-55 fixed bottom-20 right-7 md:bottom-24 md:right-10 lg:bottom-28 lg:right-12 '><FaArrowCircleUp /> </button>
					)}
				</div>
				<Outlet />
				<Footer />
